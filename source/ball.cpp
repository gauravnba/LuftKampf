...
BallC::BallC(float32_t initPosX,float32_t initPosY,float32_t initVelX,float32_t initVelY,int id)
{
...
void BallC::CollideField(void)
{
	...
	float32_t rightSide = field->getPosition()->x+((float32_t)field->getWidth()/2.0f);
	float32_t leftSide = field->getPosition()->x-((float32_t)field->getWidth()/2.0f);
	float32_t topSide = field->getPosition()->y+((float32_t)field->getHeight()/2.0f);
	float32_t bottomSide = field->getPosition()->y-((float32_t)field->getHeight()/2.0f);
...
void BallC::render()
{
	...
	//{
	//case StateManager::ALL_BALLS_FILLED:
	//	filledVal = true;
	//	break;
	//case StateManager::HALF_BALLS_FILLED:
	//	filledVal = (mID % 2) ? true : false;
	//	break;
	//case StateManager::NO_BALLS_FILLED:
	//	filledVal = false;
	//	break;
	//}
