...
FieldC::FieldC(float32_t x, float32_t y, uint32_t height, uint32_t width, uint32_t initColor)
{
    mEnabled = true;
...
void FieldC::render()
{
	float32_t left = mPosition.x - (float32_t)mWidth/2.0f;
	float32_t right = mPosition.x + (float32_t)mWidth/2.0f;
	float32_t bottom = mPosition.y - (float32_t)mHeight/2.0f;
	float32_t top = mPosition.y + (float32_t)mHeight/2.0f;
